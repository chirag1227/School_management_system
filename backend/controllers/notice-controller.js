...
const noticeCreate= async (req,res)=>{
   try {
    ...
    console.log("School (adminID):", req.body.school); // Log the specific adminID
...
const updateNotice=async (req,res)=>{
    try {
        let result=await Notice.findByIdAndUpdate(req.params.id,
            {$set:req.body},
            {new:true})
...
const deleteNotices=async (req,res)=>{
    try {
        ...
        if(result.deletedCount===0){
            res.send({message:"No notice found to delete"});
        }
        else {
            res.send(result);
        }
